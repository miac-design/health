...
function SuppCard({ p, onClick }) {
  return (
    <div className="card" style={{ cursor: 'pointer', display: 'flex', gap: 14 }} onClick={onClick}>
      <ProductThumb product={p} size={70} emoji="💊" />
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div className="s-meta" style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginTop: 8 }}>
          {p.dose && <span className="badge neutral">💊 {p.dose}</span>}
          {p.bestTime && <span className="badge">🕐 {p.bestTime}</span>}
          {p.withFood && p.withFood !== 'Either' && <span className="badge warn">🍽 {p.withFood}</span>}
        </div>
        {p.benefits && (
        ...
        {(p.interactions || p.whenNotToTake) && (
          <div className="tiny" style={{ marginTop: 6, color: 'var(--warn)' }}>
            ⚠️ {[p.interactions, p.whenNotToTake].filter(Boolean).join(' · ')}
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <>
      ...
      {supplements.length === 0 ? (
        <Empty emoji="💊" title="No supplements yet"
      ...
      <button className="fab" onClick={() => setAdding(true)} aria-label="Add supplement">+</button>
