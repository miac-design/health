...
Routes: `/#/`, `/#/products`, `/#/routine/{morning,evening,shower}`, `/#/supplements`, `/#/health`, `/#/habits`, `/#/wishlist`, `/#/progress`, `/#/notes`, `/#/settings`.

## Flows worth driving

- Home: `+ Add glass` bumps the water ring; tiles show live counts.
...
- Routine: click a `.flow-step .dot` to check off (ring updates); "✎ Edit routine" → link a library product to a step (its photo replaces the emoji).
- Health: steppers/scales write straight to IndexedDB — reload the page to confirm persistence.
- Habits: `.habit-check` toggle updates the ring and week dots.
- Progress: upload two photos, "⚖️ Compare two", pick both → Before/After captions.
- Settings: "Download backup" fires a real download — parse the JSON and check table counts.

## Gotchas

- Each Playwright launch is a fresh profile → clean IndexedDB, seed data (21 routine steps, 9 habits) re-created on load.
- `page.click('text=…')` is non-strict and can hit a modal *title* with the same text as its submit button — use `button:has-text(…)`.
